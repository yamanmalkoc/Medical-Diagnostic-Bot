##Reference: This API is a modified implementation of the one at https://github.com/JMW95/pyST7920
...
class ST7920:
	'''
	Initializes the ST7920 display and loads a fontsheet to use
	character fonts from
	'''
	def __init__(self):
		self.spi = spidev.SpiDev()
		self.spi.open(0,0)
		self.spi.cshigh = True # use inverted CS
		self.spi.max_speed_hz = 125000 # set SPI clock to 1.8MHz, up from 125kHz
	...
	'''
	Allows user to rotate the characters displayed
	'''
	...
	'''
	Loads the font from a fontsheet.png in the 
	directory
	'''
	...
	'''
	Sends binary data to interface with the LCD through
	serial connection and SPI
	'''
	...
	'''
	Makes a filled rectangle, essentially representing a 
	larger pixel block
	'''
	...
	'''
	Plots a point based off its coordinates
	'''
		...
		for c in s:
			try:
				...
				for row in char:
					sx = 0
					for px in row:
						self.fill_rect(x+sx, y+sy, x+sx+1, y+sy+1, px == 1)
						sx += 1
					sy += 1
			except KeyError:
				pass
	...
	'''
	Writes text of an adjustable size onto the display
	'''
    def put_text_size(self, s, x, y, a):
		for c in s:
			try:
				font, cw, ch = self.fontsheet
				char = font[ord(c)]
				...
				for row in char:
					sx = 0
					for px in row:
						self.fill_rect(x+sx, y+sy, x+sx+a, y+sy+a, px == 1)
						sx += a
					sy += a
			except KeyError:
				pass
	...
	'''
	Refreshes the display and updates to show recent changes
	'''
